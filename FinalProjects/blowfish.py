import numpy as np
from numbapro import cuda, jit, uint64
from numba import *
import time 
...
def InitializeBlowfish(key, keybytes):
	j = 0
	...
	for i in range(0, N+2, 2):
		datal, datar = Blowfish_encipher(datal, datar)
		P[i] = datal
		P[i+1] = datar
...
try:
	f = open('Test2Image.jpg', 'rb')
	fileI = np.fromfile(f, dtype='uint32')
	f.close()
except IOError:
...
lengthI = len(fileI)
timer = time.time()
for i in range(0, lengthI/4 + 1, 2):
	fileI[i], fileI[i+1] = Blowfish_encipher(fileI[i], fileI[i+1])

runtime = time.time() - timer
print("encipther takes %fs" % runtime)

for i in range(0, lengthI/4 + 1, 2):
	fileI[i], fileI[i+1] = Blowfish_decipher(fileI[i], fileI[i+1])


fileI.tofile('new.jpg')